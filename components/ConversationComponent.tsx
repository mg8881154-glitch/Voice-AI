...
import { AgentImageCard } from './AgentImageCard';
import { useLeadExtractor } from '@/lib/useLeadExtractor';
import { useImageTrigger } from '@/lib/useImageTrigger';
...
export default function ConversationComponent({
  ...
  // Auto-extract lead data from completed user turns.
  // Runs after every transcript update; merges new findings into LeadContext
  // and reactively recomputes the qualification status.
  useLeadExtractor(transcript, String(client.uid));

  // Watch AGENT turns for product/feature keywords and surface a matching image.
  const { currentImage, dismissImage } = useImageTrigger(transcript, agentUID);
  ...
  return (
    <QuickstartConversationLayout
      ...
      imageCard={
        currentImage ? (
          <AgentImageCard image={currentImage} onDismiss={dismissImage} />
        ) : null
      }
      visualizer={
        <div
